...
def combobox_decoder_code():
    ...
    if combobox_value == "true":
        ...
        key = file_key.read()
        file_key.close()
        final_text = enter_decoder_code(key)
        final_text = hamster_language(enter_value)
        messagebox.showinfo("Final A", final_text)

    if combobox_value == "false":
        message = hamster_language(enter_value)
        file_key = open("Key", "w+")
        ...
        final_text = enter_decoder_code(key)
        final_text = hamster_language(enter_value)
        messagebox.showinfo("Final B", final_text)
